...
def scan_csv_gz(path: Path, cfg: FileScanConfig) -> Dict[str, Any]:
    ...
    for chunk in pd.read_csv(path, compression="gzip", chunksize=cfg.chunksize):
        if not columns:
            columns = [c for c in chunk.columns if c != "Unnamed: 0"]
        ...
        cols_to_check = [c for c in chunk.columns if c != "Unnamed: 0"]
        if cols_to_check:
            chunk_hashes = pd.util.hash_pandas_object(chunk[cols_to_check], index=False)
